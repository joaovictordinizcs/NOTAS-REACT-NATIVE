...
export default class ItemDatabase {
  Conectar() {
    //**** CRUD => CREATE - aqui o BD é aberto e a tabela é criada se não existir *****/
    let db;
        ...
        .then(() => {
          ...
          SQLite.openDatabase(
            ...
            .then(DB => {
              ...
              db.executeSql('SELECT 1 FROM Item LIMIT 1')
                .then(() => {
                  console.log(
                ...
                .catch(error => {
                  ...
                  db.transaction(tx => {
                    ...
                    tx.executeSql(
                        'CREATE TABLE IF NOT EXISTS Item (id INTEGER PRIMARY KEY AUTOINCREMENT, nome varchar(30),turno varchar(30),materia varchar(30), nota DOUBLE, data DOUBLE ,status varchar(30))',
                      );                      
                  })
                    .then(() => {
  ...
  Listar() {
    //**** CRUD => READ - aqui a tabela é lida *****/
    return new Promise(resolve => {
      const lista = [];
      this.Conectar()
        .then(db => {
          db.transaction(tx => {
            ...
            tx.executeSql('SELECT * FROM Item', []).then(([tx, results]) => {
              ...
              for (let i = 0; i < len; i++) {
                ...
                lista.push({id, nome, turno, materia, nota, data, status});
              }
              console.log(lista);
              resolve(lista);
            });
  ...
  Inserir(item) {
    //**** CRUD => UPDATE - aqui um registro da tabela é inserido *****/
    return new Promise(resolve => {
      this.Conectar()
        .then(db => {
          db.transaction(tx => {
            ...
            tx.executeSql(
              'INSERT INTO Item (nome, turno, materia, nota, data, status) VALUES (?, ?, ?, ?, ?, ?)',
              [item.nome, item.turno, item.materia, item.nota, item.data],
            ).then(([tx, results]) => {
              resolve(results);
            });
          })
  ...
  Atualizar(item) {
    //**** CRUD => UPDATE - aqui a tabela é atualizada *****/
    return new Promise(resolve => {
      this.Conectar()
        .then(db => {
          db.transaction(tx => {
            //Query SQL para atualizar um registro no banco
            tx.executeSql('UPDATE Item SET status = ? WHERE id = ?', [
              item.status,
              item.id,
            ]).then(([tx, results]) => {
              resolve(results);
  ...
  Remover(id) {
    //**** CRUD => DELETE - aqui um registro da tabela é removido *****/
    return new Promise(resolve => {
      this.Conectar()
        .then(db => {
          db.transaction(tx => {
